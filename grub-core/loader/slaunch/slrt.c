                        ...
                        struct grub_slr_policy_entry *platform_entry)
{
  ...
  if (boot_params != NULL)
    {
      efi_info = (struct grub_efi_info *)
              ((grub_uint8_t *)&boot_params->efi_info.v0208 +
  ...
  if (boot_params != NULL && boot_params->hdr.setup_data)
    {
      slr_policy_staging->policy_entries[i].pcr = 18;
      slr_policy_staging->policy_entries[i].entity_type = GRUB_SLR_ET_SETUP_DATA;
      slr_policy_staging->policy_entries[i].entity = boot_params->hdr.setup_data;
      slr_policy_staging->policy_entries[i].flags |= GRUB_SLR_POLICY_IMPLICIT_SIZE;
  ...
  if (boot_params != NULL && boot_params->hdr.cmd_line_ptr)
    {
      slr_policy_staging->policy_entries[i].pcr = 18;
      slr_policy_staging->policy_entries[i].entity_type = GRUB_SLR_ET_CMDLINE;
      slr_policy_staging->policy_entries[i].entity = boot_params->hdr.cmd_line_ptr;
      hi_val = boot_params->ext_cmd_line_ptr;
      slr_policy_staging->policy_entries[i].entity |= hi_val << 32;;
      slr_policy_staging->policy_entries[i].size = boot_params->hdr.cmdline_size;
  ...
  if (boot_params != NULL && boot_params->hdr.ramdisk_image)
    {
      slr_policy_staging->policy_entries[i].pcr = 17;
      slr_policy_staging->policy_entries[i].entity_type = GRUB_SLR_ET_RAMDISK;
      slr_policy_staging->policy_entries[i].entity = boot_params->hdr.ramdisk_image;
      hi_val = boot_params->ext_ramdisk_image;
      slr_policy_staging->policy_entries[i].entity |= hi_val << 32;
      slr_policy_staging->policy_entries[i].size = boot_params->hdr.ramdisk_size;
...
{
  grub_dprintf ("slaunch", "DLME size: 0x%llx\n", (unsigned long long)slparams->mle_size);
  grub_dprintf ("slaunch", "DLME entry point: 0x%x\n", (unsigned int)slparams->mle_entry);
  ...
  for (i = 0; i < policy->nr_entries; i++)
    {
      if (policy->policy_entries[i].entity_type == GRUB_SLR_ET_UNUSED)
        {
          if (next == 0 && boot_params->hdr.setup_data)
            {
              policy->policy_entries[i].pcr = 18;
              policy->policy_entries[i].entity_type = GRUB_SLR_ET_SETUP_DATA;
              policy->policy_entries[i].entity = boot_params->hdr.setup_data;
          ...
          else if (next == 1 && boot_params->hdr.cmd_line_ptr)
            {
              policy->policy_entries[i].pcr = 18;
              policy->policy_entries[i].entity_type = GRUB_SLR_ET_CMDLINE;
              policy->policy_entries[i].entity = boot_params->hdr.cmd_line_ptr;
              hi_val = boot_params->ext_cmd_line_ptr;
              policy->policy_entries[i].entity |= hi_val << 32;;
              policy->policy_entries[i].size = boot_params->hdr.cmdline_size;
          ...
          else if (next == 3 && boot_params->hdr.ramdisk_image)
            {
              policy->policy_entries[i].pcr = 17;
              policy->policy_entries[i].entity_type = GRUB_SLR_ET_RAMDISK;
              policy->policy_entries[i].entity = boot_params->hdr.ramdisk_image;
              hi_val = boot_params->ext_ramdisk_image;
              policy->policy_entries[i].entity |= hi_val << 32;
              policy->policy_entries[i].size = boot_params->hdr.ramdisk_size;
              hi_val = boot_params->ext_ramdisk_size;
              policy->policy_entries[i].entity |= hi_val << 32;
